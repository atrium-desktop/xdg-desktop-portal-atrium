//! Routing regression: OpenURI is served by Tessera.
...
#[test]
fn open_uri_is_served_by_tessera() {
    ...
    assert!(
        interfaces
            .split(';')
            .any(|interface| interface == "org.freedesktop.impl.portal.OpenURI"),
        "the Portal-owned OpenURI must be advertised"
    );
    assert!(
        PORTALS_CONF
            .lines()
            .any(|line| line == "org.freedesktop.impl.portal.OpenURI=tessera")
    );
}

...
`atrium-portal-prompter` is the one-shot, out-of-process user interface host
...
The dialogs follow the tessera design language (the palette is mirrored
locally — the portal build graph stays independent of the Tessera repository)
...
development against a sibling optics checkout mirrors the Tessera workflow:
...
```bash
cargo run -p atrium-portal-prompter << 'EOF'
{
  "version": 6,
  "prompt": {
    "kind": "file_chooser",
...
cargo run -p atrium-portal-prompter << 'EOF'
{
  ...
  "prompt": {
    ...
    "request": {
      "title": "Unlock Vault",
      "reason": "dev.tessera.Test requires your master password."
    }
  }
}
EOF

...
use atrium_portal_prompter::{FileFilter, FilterRuleKind};
...
mod tests {
    use super::*;
    use atrium_portal_prompter::FilterRule;
...
";
    ...
    fn listing_sorts_dirs_first_and_hides_dotfiles() {
        let root = std::env::temp_dir().join(format!("tessera-chooser-{}", std::process::id()));
    ...
    fn bookmarks_parse_and_roundtrip() {
        let root = std::env::temp_dir().join(format!("tessera-bookmarks-{}", std::process::id()));

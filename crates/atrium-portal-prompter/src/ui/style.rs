//! The tessera product look, mirrored locally: palette, theme factory, layout
//! metrics, and the shared style atoms for prompt surfaces. The portal build
//! graph stays independent of the Tessera repository, so the token *values*
//! are duplicated here instead of imported from `tessera-design`.
//!
...
pub mod metrics {
    ...
    /// `tessera-design`'s `radii.card` (16).
...
/// One tessera scheme's prompt-surface tokens (values mirror
/// `tessera-design`'s `Colors`, except `danger`, which has no counterpart
...
pub struct Palette {
    ...
    /// opaque surface. Mirrors `tessera-design`'s `card_surface`.
    pub material: Color,
    ...
    /// `tessera-design`'s `scrim`.
    #[allow(dead_code)]
    pub scrim: Color,
}

/// The dark tessera appearance (`Design::dark`).
pub fn dark() -> Palette {
    Palette {
...
/// The light tessera appearance (`Design::light`).
...
impl ThemeInput {
    ...
    fn from_parts(snapshot: Option<&PromptAppearance>, platform_dark: bool) -> Self {
        use atrium_portal_prompter::PromptColorScheme;
...
/// The lens theme for prompt surfaces: the tessera palette over the matching
